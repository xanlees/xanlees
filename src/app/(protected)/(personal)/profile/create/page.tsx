...
import { employeeProfileStorageKey, ProfileProvider, useProfileContext } from "../../context";
...
export function ProfileCreate({ params }: { params: { id: number } }): JSX.Element {
...
const FormCreate = ({ userID }: { userID: number }) => {
  const router = useRouter();
  const { state, dispatch } = useProfileContext();
  const [isWaiting, setIsWaiting] = useState(false);
  const initialStep = getStepState(state);
  const formStepsData = getFormStepsData({ userID });
...
const getFormStepsData = ({ userID }: { userID: number }) => [
  { stepLabel: "ຂໍ້ມູນສ່ວນບຸກຄົນ", stepDescription: <ProfileForm isEmployee={true} type="EMPLOYEE" createUserProfile={true} user={userID} />, completed: false },
